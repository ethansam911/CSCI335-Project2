/**
	...
	This AvlTree Class provides functions to access and modify
	the data of the AvlTree and the AvlTree itself
...
class AvlTree
{
  public:
	...
	/**
      ...
      */
    int heightOfTree(int height) const 
	{
  ...
  private:
	...
	int findHeight( AvlNode *t, int height) const
    {
         if (t == nullptr)
            return -1;
        else
            return height + findHeight(t->left, height+1) + findHeight(t->right, height+1);
    }
    ...
    int totalNodes( AvlNode *t )const 
	{
        int count = 0;
        if( t != nullptr ) {
            count = 1 + totalNodes( t->left ) + totalNodes( t->right );
        }
        return count;
    }
    ...
    int findDepth(AvlNode *t, int depth) 
	{
        if (t == nullptr)
            ...
            return depth + findDepth(t->left, depth+1) + findDepth(t->right, depth+1);
    }
    ...
    void findAndPrint( const Comparable &x, AvlNode *t ) 
	{
        if( t == nullptr )
            return;
        else if( x < t->element )
            return findAndPrint( x, t->left );
        else if( t->element < x )
            return findAndPrint( x, t->right );
        else
...
*****************************************************/
     ...
     */
	 // Made height of node for no reason, this one already exists
